...
bool aheadEnough(){
    //factor in if they're moving or taking an item back to their zone
    ...
    -(myDeltaScore * dist(enZone, myPos) * 5) && myScore - enScore != 0.1f && dist(enPos, enZone) > .4){
        mathVecSubtract(enToZone, enPos, enZone, 3);
        mathVecNormalize(enToZone, 3);
        if(angle(enVel, enToZone) > .05f || 1.25 * dist(myPos, enZone) < dist(enPos, enZone))
            return true;
    }
    return false;
}
