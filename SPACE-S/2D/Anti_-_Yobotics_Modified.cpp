...
float enDeltaScore;
bool newItemInZone;
float myPredictedScore;
float enPredictedScore;
bool carryingItem;
...
void loop(){
    ...
    myPredictedScore = (180-time)*myDeltaScore + myScore;
    enPredictedScore = (180-time)*enDeltaScore + enScore;
    ...
    if (pickNewTargetItem)
            targetItem = bestItem();
    // calculating whether or not to protect the zone
    // and what point to protect
    if(myDeltaScore>=0.39 and myPredictedScore > enPredictedScore) {
    ...
    // getting and carrying item
        ...
        if(!carryingItem) {
            ...
            // calculating the distance to zone and getting there 
