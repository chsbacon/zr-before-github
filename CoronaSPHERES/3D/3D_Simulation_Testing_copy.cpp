//Begin page Flare
bool flareImminent() {
    float vecAstToUpload[3];
    float vecUploadToSphere[3];
    vecManip(vecAstToUpload, myPos, 0, zeroVec);
    mathVecNormalize(vecAstToUpload, 3);
    vecManip(vecAstToUpload, zeroVec, 0.53, vecAstToUpload);
    vecManip(vecUploadToSphere, myPos, -1.0, vecAstToUpload);
    float runTime = 11 + (1000*mathVecInner(vecUploadToSphere, myVelo, 3));
    if ((((nextFlare < runTime && nextFlare != -1) 
            || gameTime > 230)
...
bool closeToUpload() {
    ...
    return (((cosf(theta)*mathVecMagnitude(myVelo, 3)*4 > distToUpload)
            && game.getMemoryFilled() > 0));
}
...
void checkAntiFlareTrigger() {
	...
	float myFuturePos[3];
	vecManip(myFuturePos, myPos, 5.0, myVelo);
 	if (nextFlare == 1 && mathVecMagnitude(myFuturePos, 3) > 0.62
 	   && mathVecMagnitude(myFuturePos, 3) < 0.35) {
 	    game.turnOff();
 	    game.turnOn();
 	}
}
...
void moveToPoint(float targetPos[3]) {
    ...
    findClearPath(targetPos, zeroVec, ASTAVOIDANCERADIUS);
    if (enemyInPath(targetPos)) {
        findClearPath(targetPos, enemyPos, ENEMYAVOIDANCERADIUS);
...
bool enemyInPath(float point[3]) {
    ...
    if (mathVecMagnitude(vecToEnemy, 3) * sinf(theta) < ENEMYAVOIDANCERADIUS
...
void moveToUploadPhoto() { 
	...
	vecManip(escapeVec, zeroVec, 0.54, escapeVec);
...
bool POISwitchClose() {
    return  ((gameTime < 230 && 60 - (gameTime % 60) < 10)
            && distance(myPos, photoSpots[indexPhotoSpot]) > 0.15); 
}
...
void rotateToPOI() {
    ...
    vecManip(POItoSphere, myPos, -1.0, POILoc);
    ...
    if (distance(myPos, enemyPos) < 0.3 
...
bool inPictureTakingPosition() {
    ...
    if (angleCalc(myOrientation, attTarget) > 0.25 && !(flareImminent()) && gameTime > 30
...
void init() {
    ASTAVOIDANCERADIUS = 0.45;
    ENEMYAVOIDANCERADIUS = 0.3;
    SPEEDCONSTANT = 1.75;
    gameTime = -1;
    zeroVec[0] = 0;
    zeroVec[1] = 0;
...
void loop(){
	...
	if ((nextFlare <= 5 && nextFlare != -1) && (antiFlareCodeTrigger))
	    ...
	    else if (POISwitchClose()) {
	        if (game.getMemoryFilled() > 0) {
	            moveToUploadPhoto();
	            DEBUG(("CODE TRIGGERED"));
	        }
	        else
	            moveToPOISwitchSpot();
	    }
