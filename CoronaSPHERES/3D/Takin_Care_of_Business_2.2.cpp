...
void uploadSpots() {
    ...
    for (int i = 0; i < 3; i++) {
        game.getPOILoc(POI, i);
        mathVecNormalize(POI, 3);
        vecManip(POIAdj, zeroVec, 0.6, POI);
        vecManip(uploadSpots[i], POIAdj, 0.0, zeroVec);
    }
}
...
int smartRoute() {
    float spots[4][3];
    float spotTemp[3];
    float routeScore[9][9][9][9][3]; //this is much data, may change to int to save mem
    int count = 0;
    int mem = game.getMemoryFilled();
    int memCap = game.getMemorySize();
    bool eligble[4];
    
    for (int i = 0; i < 9; i++) {
        if (i < 6)
            vecManip(spotTemp, photoSpots[i], 0.0, zeroVec)
        else 
            vecManip(spotTemp, uploadSpots[i-6], 0.0, zeroVec)
        //if point is right type (photo or upload spot)
        //and if point is safe
        vecManip(spots[count], photo/uploadSpot[i], 0.0, zeroVec)
        for (int j = 0; j < 9; j++) {
            //if point is right type (photo or upload spot)
            //and if point is unused
            vecManip(spots[count], photo/uploadSpot[j], 0.0, zeroVec)
            for (int k = 0; k < 9; k++) {
                //if point is right type (photo or upload spot)
                //and if point is unused
                vecManip(spots[count], photo/uploadSpot[k], 0.0, zeroVec)
                for (int l = 0; l < 9; l++) {
                    //if point is right type (photo or upload spot)
                    //and if point is unused
                    vecManip(spots[count], photo/uploadSpot[l], 0.0, zeroVec)
                    routeScore[i][j][k][l][0] = //calculate dist between 4 points
                    routeScore[i][j][k][l][1] = //calculate points collected on route
                    routeScore[i][j][k][l][2] = //caclulate total angle change
...
bool inPictureTakingPosition() {
	...
	//DEBUG(("Facing POI Angle %f\n", angleCalc(myOrientation, attTarget)));
	//DEBUG(("DistToAsteroid %f\n", dist));
	//DEBUG(("Cone (Angle Variance) %f\n", angleCalc(POILoc, POItoSphere)));
	//DEBUG(("POIID %i", POIID));
	//DEBUG(("\nindexPhotoSpot %i", indexPhotoSpot));
...
//Improve weighting
//60s and Flare in weighting
//PHOTOTAKING BUG HAS NOT BEEN FIXED AS OF TIME WRITING THIS
