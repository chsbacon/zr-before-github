...
void loop(){
    ...
    float timeToTop = 10*angleCalc(vecCircToPOI, zVec);
    ...
    if ((game.getMemoryFilled() > 0) && (nextFlare < 15 || 
        ...
        upload = 1;
            ...
            || timeToLattitude(chosenPOILoc) > 4 || fuelRemaining == 0) {
    ...
    if ((nextFlare <= 15 || realTimeToLat(chosenPOILoc, chosenPOIID) + 15 > nextFlare)
        || fuelRemaining < 7) {
        float darkSpot[3] = {0.45f, 0.1f, -0.1f};
        if (!(blue))
         	darkSpot[1] *= -1;
        if (bottom)
    	    darkSpot[2] *= -1;
    ...
    if ((myPos[0] < 0 || myPos[1]*myPos[1] + myPos[2]*myPos[2] > 0.2) && nextFlare == 1) {
        game.turnOff();
        game.turnOn();
    }
}
...
void moveToPoint(float targetPos[3]) {
	...
	vecManip(velocityTarget, zeroVec, 0.025f, velocityTarget);
