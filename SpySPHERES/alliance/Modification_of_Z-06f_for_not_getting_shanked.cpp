...
bool                must_upload_pictures, oponent_in_dark, //(0 for blue, 1 for red)
                    oponent_close, slow_down, pack_detected,
...
void loop(){
	...
	vcoef = (light!=LIGHT_AREA) ? 0.3f : 0.55f;
    ...
    label4:
        if (score_item>=0){
            ...
            if (  ((tt<8)&(distance_to_light<0.4f)&(distance_to_light>-0.05f)&(o_distance>0.4f))
    ...
    case 6:
        ...
        if (mirror_item>=0){
            ...
            if (o_distance<0.3f)
                vcoef = .4f;
            if ((theirDist>.01f and opos[0]*destination[0]>0/*Are they on the same side as the mirror*/ and ourDist>.03f and (theirDist>ourDist-.2f and theirDist<ourDist+.4f)))/// MAJOR CHANGE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                vcoef = .450f;
        }
        else{
            //step = 8;
    ...
    case 8://go up for pictures
        ...
        destination[2] = -0.35f;
        destination[0] = 0.0f;
        if ((TIME<130)&(light==DARK_AREA)&(!oponent_in_dark)&(opos[1]-cpos[1]<0.55f)){
