...
//new
    ...
    */
    ...
    case 2: //Score+ selection. Set destination for item picking
        ...
        if ((destination[2] - opos[2]>0.40f)|(ovel[1]<-0.01f) ){
            label_move_away:
            if (TIME>40){
                if ((ovel[1]<-0.01f)&(opos[1]<0.45f)) {
                    step = 6;
                    goto label6;
                }
            }
    ...
    case 8://go up for pictures
        label8:
        if (score_item>=0){
            game.getItemLoc(fvector,score_item);
            destination[2] = fvector[2];
        }
        else
            destination[2] = -0.40f;
        destination[0] = (cpos[0]>0) ? 0.45f : -0.45f;
        destination[1] = (distance_to_light<0.91f) ? light_border-0.1f : 0.54f;
        if ((TIME>141)&(score_item>=0)){
            memcpy(destination, fvector, VEC_SIZE);
        }
        else{
            if (TIME>156){
                game.useMirror();
	...
	//safer
	if  (((TIME>90)&oponent_close&((velocity<0.01)|(light==LIGHT_AREA))) 
	    /*|((energy>2.8f)&(tt>10))*/ )
    ...
    //mirror code
    if ( (distance_to_light<0.88f)&(o_distance>0.44f)&(TIME>80) ){
