...
void moveToPoint(float targetPos[3]) {
    ...
    scale(velocityTarget, (dist < 0.14f) ? dist*dist : 0.022f);
...
void loop()
{
    ...
    //get energy
    else if (myEnergy<3&&game.hasItem(closeEnergy)==-1&&gameTime > 120){
    ...
    if (game.hasItem(closeMirror)==0 && myPos[1]-lightTail<-0.05f && lightFront>0) firstLightPassWithMirror=false;
    
    DEBUG(("lightTail:%f myPos[1]-lightTail:%f first pass:%d",lightTail,myPos[1]-lightTail,firstLightPassWithMirror));
