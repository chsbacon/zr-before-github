...
void loop()
{
	...
	if ((gameTime<45 || myPosInArea!=1)&&closePP!=-1&&game.getScore()<3.0&&!abandoningPPs) {
        ...
        abandoningPPs=true;
        if (game.getMemoryFilled() == 2 && myPosInArea != -1) {
            float getAwaySpot[3];
            getAwaySpot[0] = myPos[0];
            getAwaySpot[1] = myPos[1] - 0.2f;
            getAwaySpot[2] = myPos[2];
            api.setPositionTarget(getAwaySpot);
        }
            
        if (game.hasItem(closeMirror)==0&&myPosInArea==-1) firstLightPassWithMirror=false;
        
    }
    else if ((game.getNumMirrorsHeld()>0||game.getMirrorTimeRemaining()>0) && myDistToLightFront < 0.25f){
        DEBUG(("going for energy waypoint and deploying mirror"));
        if (!firstLightPassWithMirror&&game.getNumMirrorsHeld()>0&&myDistToLightFront < 0.05f);
            game.useMirror();
        float target[3];
    ...
    else if (myDistToLightFront < -0.9f)/*if (game.hasItem(closeEnergy)==-1)*/{
        DEBUG(("going for energy"));
        float vecToEnergy[3];
        mathVecSubtract(vecToEnergy, ItemLoc[closeEnergy], myPos, 3);
        float vecLength = mathVecMagnitude(vecToEnergy, 3);
        if (vecLength > 0.2f) {   
            scale(vecToEnergy, 0.03f); // vecToEnergy is not normalized. why?
            api.setVelocityTarget(vecToEnergy);
        }
        else
...
// 	DEBUG(("energy usage: us:%f them:%f", game.getEnergy()-myEnergy, game.getOtherEnergy()-enEnergy));
    ...
    if(lightFront>0.84) lightFront-=1.68; // if it's out of bounds, correct it
