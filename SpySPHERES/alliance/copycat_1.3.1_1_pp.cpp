...
void loop()
{
    ...
    //get point-packs    
	if ((gameTime<45 || myPosInArea!=1)&&pps<1&&!abandoningPPs) {
...
// 	pic logic (37%)
	if (gameTime>20&&(game.isFacingOther() and distanceBetween(myPos,enPos)>PHOTO_MIN_DISTANCE 
        and (myEnergy>1.6f or myPosInArea==1) and enPosInArea!=-1
        and memoryFilled<2 && game.getMirrorTimeRemaining() == 0)
        or myEnergy < 1.0f or (game.hasItem(closePP)!=-1 and gameTime>150)){
