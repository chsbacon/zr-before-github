...
bool                mirror, oponent_in_dark, //(0 for blue, 1 for red)
                    oponent_close, hardstop, slow_down,
                    can_take_a_picture, oponent_converges, wait_flag;
short               light, score_item, time_to_switch, old_score, old_energy;
float               destination[3], *cpos, *cvel, *catt, *crot, *oatt,
                    ...
                    desired_attitude[3], dark_border, light_border, ovelocity, distance_to_light;
...
void loop(){
    ...
    oponent_converges = ((o_distance<0.5f)&&(flocal > o_distance)) ? true : false;
    #ifdef MOV_DEBUG_MESSAGES2
    DEBUG(("Converges %d.", oponent_converges));
    #endif

    oponent_close = (o_distance<0.35f) ? 1 : 0;
    #ifdef MOV_DEBUG_MESSAGES2
	...
	distance_to_light = cpos[1] - light_border;
	//dark_border = (light_border>0) ? light_border - 0.8f : light_border + 0.8f;
	
	can_take_a_picture = CanTakePicture();
	...
	action_energy = ((light!=LIGHT_AREA)&&(distance_to_light>0.90f))||(distance_to_light < 0.15f)? 2.8f : 1.2f;
	
	must_upload_pictures = MustUpload();
	
    // Messages (printed only if DEBUG_MESSAGES is defined)
	#ifdef DEBUG_MESSAGES
	DEBUG(("TIME: %d, Step: %d, FUEL: %f, Energy: %f.\n", TIME, step,  fuel, energy));
	#endif
	
	vcoef = (step<=6) ? 0.8f : 0.5;
	if (light!=LIGHT_AREA){
	    vcoef = (energy>3) ? 0.7f : (energy>1) ? 0.5f : 0.2f;
	...
	if (velocity<0.02f)
	    flocal = 0.5f;
    else if (velocity<0.030f) 
        flocal = 0.70f;
    else{
        flocal = 1.2f;
    }
	hardstop = (light!=LIGHT_AREA)&&(energy<=flocal) ? 1 : 0;
	...
	energy_item = ItemSelection2(0,2);//SimpleEnergySelection();
	
	
    
    //DEBUG(("Light Border : %f", light_border));
    //DEBUG(("LightIsClosing : %d", LightIsClosing()));
    ...
    case 0:
        ...
        if ((old_energy!=energy_item)&&(TIME>18)){
            if (energy>4.0f){
                step=2;
                goto label2;
            }
        }
        /*if (action_energy>2.0) 
            action_energy =  2.0f;*/
    break;
    ...
    case 2: //Score+ selection. Set destination for item picking
	    if (score_item>=0){
	        if (old_score!=score_item){
                ...
                //if opponent is above 0.95
                if ((distance_to_light<1.05f)&&(distance_to_light>0)&&(destination[2]-opos[2]>0.48f)){
                    step = 6;
                    goto label6;
                }
                //if light is closing
                if (( distance_to_light < 0.55f )&&(distance_to_light>0.05f)&&(cpos[2]>-0.1f) ){
                    step = 0;
                    goto label0;
                }
    	    }
    ...
    case 8://go up for pictures
        ...
        destination[1] = (distance_to_light>-0.1f)&&(distance_to_light<0.8f) ? light_border-0.1f : 0.54f;
        if (mirror){
            if (TIME>141){
                destination[0] = 0.0f;
                destination[1] = 0.0f;
    ...
    //mirror code
    if ( (distance_to_light>0.5f)&&(distance_to_light<0.90f)&&(o_distance>0.44f) ){
        if ((mirror)&&(energy<1.4f)&&(TIME<172)){
            mirror = false;
            game.useMirror();
        }
    }
...
//--------------------------------------------------------------
//------------- Our own functions ------------------------------
bool    HaveToPassTheBorder(float border){
    float   temp = cpos[1]  - border;
    
    DEBUG(("TEMP = %f", temp*temp));
    DEBUG(("distance = %f", DistanceBetweenPoints(destination,cpos)));
    if (cvel[1]>0.01f)
        return false;
    if (( temp*temp < DistanceBetweenPoints(destination,cpos) + 0.20f )&&( temp>0 )){
        return true;
    }
    /*if ((cpos[2]-opos[2]>0.5f)&&(cpos[2]-light_border<0.6f))
        return true;*/
    return false;
}

bool LightIsClosing(){
    return ( distance_to_light < 0.55f )&&(distance_to_light>0.05f) ;
}

bool    BorderIsClosing(float border){
    float   temp = cpos[1] - border ;
    
    return ( temp < 0.45f )&&(temp>0) ;
}

bool OponentIsAbove(){
    //if (light == LIGHT_AREA)
    if ((distance_to_light<0.95f)&&(distance_to_light>0))
        if (destination[2]-opos[2]>0.48f)
            return true;
    return false;
}


bool        MustDeployMirror(){
    //if we are in Dark we do not need a mirror
    if (distance_to_light>0.9f)
        return false;
...
bool    MustUpload(){
    ...
    if (photos_taken==1){
        ...
        if ((light==DARK_AREA)&&(energy<2.7f))
            if ((distance_to_light>1.0f)||(distance_to_light<0))
                return true;
...
void MoveToDestination(float coef){
    ...
    if (distance_to_point>tol)
        if ((velocity<0.008f)||(!moving_towards_target)){
...
short    ItemSelection2(short i_start, short i_stop){
    ...
    for (i=i_start;i<=i_stop;i++){
        if (game.hasItem(i)==-1){
            ...
            //DEBUG(("oponent direct: %f", mathVecInner(diff,ovel,3)/ovelocity));
            if ( ((mathVecInner(diff,ovel,3) < ovelocity*0.988f)&&(d2>0.12f) )||(d1<d2*d2) )
                if (d1<=d0){
                    d0=d1;
                    i0=i;
                }
