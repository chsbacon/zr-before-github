...
bool                mirror, oponent_in_dark, //(0 for blue, 1 for red)
                    ...
                    can_take_a_picture, oponent_converges;
...
void loop(){
	...
	action_energy = ((light!=LIGHT_AREA)&&(distance_to_light>0.85f))||(distance_to_light < 0.15f) ? 2.9f : 1.0f;
	...
	vcoef = (step<=6) ? 0.8f : 0.55f;
	if (light!=LIGHT_AREA){
	    vcoef = ((energy>2.0f)&&(step<6)) ? 0.70f : (energy>1.2f) ? 0.5f : 0.3f;
	    if (must_upload_pictures)
	        vcoef = 0.3f;
	}
	if ((oponent_close)||(opos[2] - cpos[2]>0.48f))
	    vcoef = 0.5f;
    ...
    */
    ...
    case 2: //Score+ selection. Set destination for item picking
        ...
        if (destination[2] - opos[2]>0.40f){
            label_move_away:
            if ((distance_to_light<0.7f)||(ovel[1]<-0.1f)){
                step = 6;
                goto label6;
            }
        }
    ...
    case 8://go up for pictures
        ...
        if ( (photos_taken==0)&&(TIME>141)&&(tt==0) ){
             dummy = true;
        }
    break;
	...
	if ( ((energy<0.9f)&&(TIME>100))||((light==LIGHT_AREA)&&(oponent_close))
	...
	if (hardstop==0){
        ...
        float distance = mathVecNormalize(fvector, 3);
        if (distance > 0.2f) {
            flocal = distance > 0.4f ? vcoef*0.07f : vcoef*0.175f*distance;
            if (( fabs(flocal - velocity) > 0.008f)||(distance<0.4f)
                ||(step>2)||(light==LIGHT_AREA)){
    ...
    //mirror code
    if ( (distance_to_light<0.88f)&&(o_distance>0.44f) ){
        if ((mirror)&&(TIME<156)&&(mathVecInner(oatt,oponent_direction,3)<-0.80f)){
            mirror = false;
            game.useMirror();
    ...
    if (((TIME%2==0)||(energy>1.9f)||(light!=DARK_AREA)||(!must_upload_pictures))&&(energy>0.5f)&&(TIME>18) )
...
short    ItemSelection3(short i_start, short i_stop){
    ...
    for (i=i_start; i<=i_stop; i++){
        if (game.hasItem(i)==-1){
            game.getItemLoc(pos1,i);
            if ((i_start==3)&&(fabs(pos1[0])<0.13f)&&(pos1[2]<0.2f)&&(TIME<30)){
                #ifdef MOV_DEBUG_MESSAGES
                DEBUG(("Pack Detected"));
                #endif
                return i;
