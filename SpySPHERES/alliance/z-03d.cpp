...
short               light, score_item, time_to_switch, old_score;
float               destination[3], *cpos, *cvel, *catt, *crot, *oatt,
                    ...
                    desired_attitude[3], dark_border, light_border, distance_to_light;
...
void loop(){
	...
	vcoef = (step<=6) ? 0.9f : 0.5f;
	if (light!=LIGHT_AREA){
	    vcoef = ((energy>3.0f)&&(step<6)) ? 0.9f : (energy>1.5f) ? 0.5f : 0.3f;
    ...
    case 8://go up for pictures
        ...
        destination[1] = (distance_to_light>-0.1f)&&(distance_to_light<0.90f) ? light_border-0.1f : 0.54f;
        if (mirror){
            if (TIME>141){
                destination[0] = 0.0f;
                destination[1] = 0.0f;
    ...
    if ( (distance_to_light>0.5f)&&(distance_to_light<0.90f)&&(o_distance>0.44f) ){
        if ((mirror)&&(energy<1.8f)&&(TIME<156)){
            mirror = false;
            game.useMirror();
        }
    }
    ...
    if ( ((TIME%2==0)||(energy>1.9f))&&(energy>0.7f) )
...
void MoveToDestination(float coef){
    ...
    if (distance_to_point>tol)
        if ((velocity<0.02f)||(!moving_towards_target)){
...
void MoveToDestinationBACON2(float vcoef){
    float usToIt[3];
    mathVecSubtract(usToIt, destination, cpos, 3);
    float distance = mathVecNormalize(usToIt, 3), des_vel=vcoef*0.07f;
    if (distance > .2f) {
        if (fabs(velocity-des_vel)>0.008f){
        ...
        DEBUG(("POSITION : vel = %f  des_vel = %f", velocity, des_vel));
        api.setPositionTarget(destination);
    }
}
...
short    ItemSelection2(short i_start, short i_stop){
    ...
    for (i=i_start;i<=i_stop;i++){
        if (game.hasItem(i)==-1){
            ...
            if ( ( ((i_start!=7)||(mathVecInner(diff,ovel,3) < mathVecMagnitude(ovel,3)*0.988f))&&(d2>0.22f))||(d1<d2*d2) )
                if (d1<=d0){
                    d0=d1;
                    i0=i;
                }
