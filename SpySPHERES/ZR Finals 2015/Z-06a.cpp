...
short               light, score_item, old_score, old_energy, tt;
...
void loop(){
	...
	bool            dummy = false, sphere_is_getting_into_light, rotation_slow=false;
	...
	sphere_is_getting_into_light = SphereInLight();
	...
	action_energy = (TIME>60) ? 1.6f : 3.8f;
    ...
    if (energy_item>=0){
        game.getItemLoc(energy_location,energy_item);
    }
	
	vcoef = (step==6) ? 0.8f : (TIME<70)? 1.0f : 0.55f;
	if (light!=LIGHT_AREA){
	    vcoef = 0.3f;
	}
	//if (oponent_close)
...
//new
    ...
    */
    ...
    case 8://go up for pictures
        label8:
        destination[2] = -0.40f;
        destination[0] = cpos[0];
        if ((TIME<130)&(light==DARK_AREA)&(!oponent_in_dark)&(opos[1]-cpos[1]<0.5f))
            destination[1] = max(-0.7, opos[1] - 0.6f);
        else
            destination[1] = sphere_is_getting_into_light ? max(-0.78, dark_border-0.8f) 
                                                        : ( (TIME<100) ? max(0.0f, cpos[1]) : 0.1f );
    break;
	...
	if (hardstop==0){
        ...
        if (distance > 0.30f) {
            ...
            if ((fabs(flocal - velocity) > 0.008f)|(step>6)){
    ...
    DEBUG(("Dark Border: %f", dark_border));
    if ( (TIME>156) | ( (sphere_is_getting_into_light)&(o_distance>0.44f)&(oenergy>1)&(TIME<156)&(mathVecInner(oatt,oponent_direction,3)<-0.80f) ) ){
        if (tt==0){
            game.useMirror();
            DEBUG(("USING MIRROR"));
        }
...
bool  SphereInLight(){
    if ((dark_border>light_border)&(distance_to_dark<0.08f)&(distance_to_light>0))
        return true;
    if ((dark_border<light_border)&(distance_to_dark<0.08f)&(distance_to_light<0))
        return true;
    return false;
