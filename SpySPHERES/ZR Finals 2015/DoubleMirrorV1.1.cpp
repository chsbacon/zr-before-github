...
void loop()
{
    ...
    && game.getMirrorTimeRemaining()==0 && enEnergy>0.49f&&(myPos[1]-lightTail>.5f))
         game.useMirror();
    ...
    //game.getMirrorTimeRemaining()>0&& sphereInLight)
    {
        ...
        waypoint[1] = distanceBetween(myPos,enPos)<0.5f ?
                                ...
                                (lightTail<0.0f&&lightTail>-0.7&&gameTime<160)? lightTail-0.1f :-0.75f;
...
// 	pic logic (37%)
	...
	if (( (myPosInArea==1 and game.getMirrorTimeRemaining()>5) 
            or (gameTime > 160) or game.getFuelRemaining()<10
            or (myPosInArea==-1 and myEnergy < 2.5f )) and memoryFilled>0){
