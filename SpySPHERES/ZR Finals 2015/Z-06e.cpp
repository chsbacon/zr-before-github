...
#define MIRROR_TIMING_CONST  26.0f // tested
...
short               light, score_item, old_score, old_energy, mirror_time_remaining, mirrors;
...
void loop(){
	...
	/*sphere_is_getting_into_light = (distance_to_dark<0.08f)&(
	...
	can_take_a_picture_no_mirror = ((mirror_time_remaining==0)&(energy>action_energy)&(photos_taken<=1)
	...
	must_upload_pictures = (photos_taken>1)
	                    |( (photos_taken==1)&(((energy<action_energy)&(distance_to_dark>0.2f))|(mirror_time_remaining>0)|(TIME>=163)) )
	                    |(must_upload_pictures&(photos_taken>0));
	
    
	mirror_time_remaining = game.getMirrorTimeRemaining();
    ...
    label4:
        if (score_item>=0){
            ...
            if ((distance_to_light<0.4f)&(distance_to_light>0)&(o_distance>0.4f))
                destination[1] = light_border - 0.05f;
        }
        else{
            step = 8;
        }
    ...
    case 8://go up for pictures
        ...
        if ((mirror_time_remaining>0) & sphere_is_getting_into_light) {
            vcoef=((cpos[1]>light_border? distance_to_light : cpos[1]+0.75f)/(mirror_time_remaining-1)) * MIRROR_TIMING_CONST;
            DEBUG(("vcoef: %f",vcoef));
        }
        //DEBUG(("oponent distance = %f", o_distance));
        //if we are close to opponent go for score items
        if ((score_item>=0)&((opos[1]-cpos[1]<0.25f)|(o_distance<0.35f))){
            step = 4;
        }
    break;
	...
	if (hardstop==0){
        ...
        if (velocity>0.0035f){
	        HardStabilize();
	       //memset(destination,0.0f,VEC_SIZE);
	       //api.setVelocityTarget(destination);
        }
	    else{
            hardstop=0;
	    }
    ...
    if (   ((sphere_is_getting_into_light)&((mirrors==2)|(TIME<50)))
              |((sphere_is_getting_into_light)&(TIME>85))  )  {       
        if (mirror_time_remaining<=1){
            game.useMirror();
            DEBUG(("USING MIRROR"));
        }
    }
...
bool    MustUpload(){
    ...
    if (photos_taken==1){
        ...
        if (((energy<action_energy)&(distance_to_dark>0.2f))|(mirror_time_remaining>0))
                return true;
...
bool SmallCanTakePicture(){
    if ((mirror_time_remaining>0)|(energy<action_energy)|(photos_taken>1)|(!game.isFacingOther())|(oponent_in_dark)|(!game.isCameraOn()))
        return 0;
    return 1;   
}
