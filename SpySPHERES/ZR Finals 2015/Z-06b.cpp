...
short               light, score_item, old_score, old_energy, tt, mirrors;
...
void loop(){
	...
	if (step>6)
	    ...
	    hardstop = (energy<0.7f) ? true : false;
    ...
    case 8://go up for pictures
        ...
        if ((TIME<130)&(light==DARK_AREA)&(!oponent_in_dark)&(opos[1]-cpos[1]<0.5f))
            ...
            destination[1] = sphere_is_getting_into_light ? max(-0.78, dark_border-0.82f) 
        ...
        if ((o_distance<0.3f)&(mirrors==2)&(cpos[1]<0)){
            step = 10;
            goto label10;
        }
    break;
    ...
    label10:
        if (score_item>=0){
            memcpy(destination,score_location, VEC_SIZE);
            if ((distance_to_light<0.4f)&(tt<5))
    ...
    //mirror code
    /*if ( (TIME>156) | ( (sphere_is_getting_into_light)&
            ( (TIME>80)|((o_distance>0.44f)&(oenergy>1)&(TIME<156)
                    &(mathVecInner(oatt,oponent_direction,3)<-0.80f))) ) ){*/
    if ( (TIME>156)|
            ((sphere_is_getting_into_light)&((mirrors==2)|(TIME<50)))|
            ( (sphere_is_getting_into_light)&
            ( (TIME>80)|((o_distance>0.41f)
                    &(mathVecInner(oatt,oponent_direction,3)<-0.80f)))) ){
        if (tt==0){
            game.useMirror();
            DEBUG(("USING MIRROR"));
