...
void loop(){
	...
	//for Hugme
	hugger = (o_distance<0.35f) ? hugger+1 : 0;
	DEBUG(("HUGGER = %d", hugger));
	
	//stop, no more energy...
	flocal = (velocity<0.02f) ? 0.70f : (velocity<0.03f) ? 0.9f : 1.3f;
	if (TIME>50)                                                            //for Hugme
	    hardstop = ((light!=LIGHT_AREA)&(energy<=flocal))|((step==8)&(hugger>4)&((oenergy>0.5f)|(energy<3.5f))) ? 1 : hardstop;
    ...
    label4:
        if (score_item>=0){
            ...
            if (  ((distance_to_light<0.4f)&(distance_to_light>-0.05f)&(o_distance>0.4f))
